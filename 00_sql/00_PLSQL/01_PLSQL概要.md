...
- 执行部分(Executable section)
  + 由关键字 BEGIN 开始，所有的可执行语句都放在这一部分，其他的 PL/SQL 块也可以放在这一部分(嵌套块)
  + 以关键字EXCEPTION 结束，如果 EXCEPTION 不存在，那么将以关键字 END 结束。
  + NULL 是一条合法的可执行语句，事物控制语句 COMMIT 和 ROLLBACK 可以在执行部分使用
  + sqlplus中执行命名的程序块必须使用 execute 关键字.
